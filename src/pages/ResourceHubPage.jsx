...
import { ChevronRight, BookOpen, Monitor, Calendar, Users, ExternalLink, X, Download } from 'lucide-react'
...
const examCards = [
  {
    tag: 'MATH', tagBg: '#e8a020', tagColor: '#fff',
  ...
  {
    tag: 'SCIENCE', tagBg: '#2d6a4a', tagColor: '#fff',
  ...
  {
    tag: 'ADVANCED', tagBg: '#1a1a1a', tagColor: '#FFDEA4',
...
const examContents = {
  'Social/Humanities Stream': [
    { id: 1, title: '2023 National Geography Final', type: 'TXT', size: '2.4 KB', date: 'May 14, 2023' },
    { id: 2, title: 'Economics Unit 4 Assessment', type: 'TXT', size: '1.1 KB', date: 'Mar 02, 2023' },
    { id: 3, title: 'History of Ethiopia - Midterm', type: 'TXT', size: '1.8 KB', date: 'Nov 18, 2022' },
    { id: 4, title: 'Civics and Ethical Education Practice', type: 'TXT', size: '900 B', date: 'Jan 10, 2023' },
  ],
  'Natural Sciences Core': [
    { id: 1, title: '2023 Physics Matriculation Exam', type: 'TXT', size: '2.1 KB', date: 'Jun 05, 2023' },
    { id: 2, title: 'Biology Lab Practical Guidelines', type: 'TXT', size: '1.9 KB', date: 'Apr 22, 2023' },
    { id: 3, title: 'Chemistry Organic Reactions Test', type: 'TXT', size: '1.4 KB', date: 'Feb 14, 2023' },
  ],
  'Mathematics (Advanced)': [
    { id: 1, title: 'Advanced Calculus Final Paper', type: 'TXT', size: '1.8 KB', date: 'May 20, 2023' },
    { id: 2, title: 'Linear Algebra Problem Set', type: 'TXT', size: '1.3 KB', date: 'Mar 15, 2023' },
    { id: 3, title: 'Statistics and Probability Mock', type: 'TXT', size: '1.9 KB', date: 'Dec 05, 2022' },
    { id: 4, title: 'Trigonometry Weekly Quiz', type: 'TXT', size: '700 B', date: 'Oct 12, 2022' },
  ]
}
...
const gatewayLinks = [
  { title: 'Afisa Academy Archive', sub: 'Global updates for Mathematics and Science topics', count: 12, href: 'https://www.africaacademy.org' },
  { title: 'YouTube EDU – Resources', sub: 'Curated external in combination for strong academic mastery', count: 8, href: 'https://www.youtube.com/education' },
  { title: 'MIT OpenCourseWare', sub: 'High-resolution and comprehensive resources for grade 12', count: 24, href: 'https://ocw.mit.edu' },
]
...
export default function ResourceHubPage() {
  ...
  const [bookingForm, setBookingForm] = useState({ date: '', time: '', purpose: '' })

  const handleDownload = (doc) => {
    const content = examQuestionBank[doc.title] || `No content available for: ${doc.title}`;
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF8F5] text-[#1a1a1a]">
      ...
      {/* ── Left Sidebar ──────────────────────────────────── */}
      <aside className="w-44 shrink-0 sticky top-0 h-screen border-r border-[#e5e1d8] bg-white flex flex-col py-6 px-4 z-20">
        {/* Brand */}
        <div className="flex items-center gap-2 mb-8 px-1">
          <div className="w-7 h-7 rounded-full bg-[#033327] flex items-center justify-center">
            <span className="text-[8px] font-bold text-[#FFDEA4]">AH</span>
          </div>
          <span className="text-[11px] font-bold text-[#033327]">Agaro High</span>
        </div>
        {/* Nav */}
          ...
          {sideNavItems.map(item => (
            <button key={item.id}
              ...
              className={`text-left text-[11px] font-semibold px-3 py-2 rounded-lg transition-colors ${
                active === item.id
                  ? 'bg-[#FFDEA4] text-[#033327]'
                  : 'text-gray-500 hover:text-[#033327] hover:bg-[#FAF8F5]'
              }`}>
              {item.label}
            </button>
          ))}
        ...
        <div className="mt-auto">
          <p className="text-[8px] text-gray-300 uppercase tracking-wider">Resource Hub v2</p>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-x-hidden">
        ...
        <section className="px-10 py-10 border-b border-[#e5e1d8]">
          <p className="text-[9px] font-bold text-[#b5985b] uppercase tracking-[0.3em] mb-3">The Living Branch</p>
          ...
          <p className="text-sm text-gray-500 max-w-lg leading-relaxed">
            Preserving the past, cultivating the future. Your gateway to the intellectual heritage of Agaro High.
          </p>
        </section>
        ...
        <section className="px-10 py-10 border-b border-[#e5e1d8]">
          <div className="flex items-start justify-between mb-1">
            <h2 className="font-serif text-xl font-bold text-[#1a1a1a]">The Exam Repository</h2>
            <span className="text-[9px] text-gray-400 mt-1">↗</span>
          </div>
          <p className="text-[11px] text-gray-400 mb-7">All Final National Examinations &amp; Preparation Assessments</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {examCards.map(card => (
              <div key={card.title}
                className="bg-white border border-[#e5e1d8] rounded-xl p-5 flex flex-col gap-3 hover:shadow-md transition-shadow">
                <span className="self-start text-[9px] font-bold px-2 py-0.5 rounded"
                  style={{ background: card.tagBg, color: card.tagColor }}>
                  {card.tag}
                </span>
                <h3 className="font-serif text-sm font-bold text-[#1a1a1a] leading-snug">{card.title}</h3>
        ...
        <section className="px-10 py-10 border-b border-[#e5e1d8] bg-white">
          <h2 className="font-serif text-xl font-bold text-[#1a1a1a] mb-1">Knowledge Vault</h2>
          <p className="text-[11px] text-gray-400 mb-8">Subject-Specific Study Materials &amp; Faculty Contributions</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {vaultStats.map(({ icon: Icon, label }) => (
              <div key={label}
                ...
                <p className="text-[11px] font-semibold text-[#1a1a1a] text-center leading-snug">{label}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="px-10 py-10 border-b border-[#e5e1d8] grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 mb-6">Curated External Learning Resources</p>
              ...
              {gatewayLinks.map(link => (
                <a key={link.title} href={link.href} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="w-9 h-9 rounded-lg bg-[#FAF8F5] border border-[#e5e1d8] flex items-center justify-center shrink-0 group-hover:bg-[#033327] group-hover:border-[#033327] transition-colors">
                    <ExternalLink size={13} className="text-[#b5985b] group-hover:text-[#FFDEA4] transition-colors" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-bold text-[#1a1a1a] leading-snug group-hover:text-[#033327] transition-colors">{link.title}</p>
                    <p className="text-[10px] text-gray-400 mt-0.5 leading-relaxed">{link.sub}</p>
                  </div>
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 mb-5">A Legacy of Knowledge &amp; Learning</p>
            {/* Library image */}
            ...
            <div className="bg-white border border-[#e5e1d8] rounded-xl p-4">
              <p className="text-[9px] font-bold text-[#b5985b] uppercase tracking-wider mb-1">Featured Read</p>
              <p className="font-serif text-sm font-bold text-[#1a1a1a] mb-1">The Electricity of Blueprint</p>
              <p className="text-[10px] text-gray-500 leading-relaxed mb-3">
                First-year reads. Curated for those who love to explore the beautiful and bibliographic stones.
              </p>
              <div className="flex gap-6 border-t border-[#e5e1d8] pt-3">
                {[["Today's Visits", "142"], ["Regular Patrons", "89"], ["Library Staff", "12"]].map(([k, v]) => (
                  <div key={k}>
                    <p className="text-[8px] text-gray-400 uppercase tracking-wide">{k}</p>
        ...
        <section className="px-10 py-12 bg-[#033327]">
          <div className="max-w-4xl flex flex-col md:flex-row gap-10">
            ...
            <div className="flex-1">
              <p className="text-[9px] font-bold text-[#FFDEA4]/60 uppercase tracking-[0.25em] mb-3">Digital Division</p>
              <h2 className="font-serif text-2xl font-bold text-white mb-4 leading-snug">ICT Lab Reservation</h2>
              <p className="text-sm text-white/60 leading-relaxed mb-8 max-w-sm">
                Manage Agaro's learning sessions for your classes. Check the day availability and leisure time slots for the upcoming academic week.
              </p>
            ...
            <div className="shrink-0 w-full md:w-56">
              <p className="text-[9px] font-bold text-[#FFDEA4]/70 uppercase tracking-[0.2em] mb-4">Today's Document</p>
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="flex items-center gap-3 bg-white/10 rounded-lg px-4 py-3">
                    <div className={`w-4 h-4 rounded-sm border flex items-center justify-center ${item.done ? 'bg-[#FFDEA4] border-[#FFDEA4]' : 'border-white/30'}`}>
                      {item.done && (
                        <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
                          <path d="M1.5 4L3.5 6L6.5 2" stroke="#033327" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-[#e5e1d8] bg-[#FAF8F5]">
              <div>
                <p className="text-[10px] font-bold text-[#b5985b] uppercase tracking-widest mb-1">Exam Materials</p>
                <h3 className="font-serif text-2xl font-bold text-[#033327]">{selectedExam}</h3>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-white">
              <div className="space-y-4">
                {examContents[selectedExam]?.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-4 border border-[#e5e1d8] rounded-xl hover:border-[#033327]/30 hover:bg-[#FAF8F5] transition-all group">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-[13px] text-[#1a1a1a] mb-1 group-hover:text-[#033327] transition-colors">{doc.title}</h4>
                        <div className="flex items-center gap-3 text-[11px] text-gray-500">
                          <span className="font-medium bg-[#e5e1d8] text-[#1a1a1a] px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider">{doc.type}</span>
                          <span>{doc.size}</span>
                          <span>•</span>
                          <span>{doc.date}</span>
                        </div>
            ...
            <div className="p-4 border-t border-[#e5e1d8] bg-[#FAF8F5] text-center">
              <p className="text-[10px] text-gray-500">
                These materials are strictly for Agaro High School students. Do not distribute externally.
              </p>
            </div>
      ...
      {isBookingModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between p-6 border-b border-[#e5e1d8] bg-[#FAF8F5]">
              <div>
                <p className="text-[10px] font-bold text-[#b5985b] uppercase tracking-widest mb-1">ICT Lab</p>
                <h3 className="font-serif text-xl font-bold text-[#033327]">Reserve a Session</h3>
              </div>
              <button 
            ...
            >
              <div>
                <label className="block text-[11px] font-bold text-[#1a1a1a] mb-1.5 uppercase tracking-wider">Date</label>
                <input required type="date" className="w-full p-2.5 bg-[#FAF8F5] border border-[#e5e1d8] rounded-lg text-sm focus:outline-none focus:border-[#033327] transition-colors" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-[#1a1a1a] mb-1.5 uppercase tracking-wider">Time Slot</label>
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#1a1a1a] mb-1.5 uppercase tracking-wider">Purpose</label>
                <textarea required rows="3" placeholder="Briefly describe the purpose of your reservation..." className="w-full p-2.5 bg-[#FAF8F5] border border-[#e5e1d8] rounded-lg text-sm focus:outline-none focus:border-[#033327] transition-colors resize-none"></textarea>
              </div>
              
              <div className="pt-4">
                <button type="submit" className="w-full py-3 rounded-lg font-bold text-sm text-white bg-[#033327] hover:bg-[#044a38] transition-colors">
                  Confirm Reservation
                </button>
              </div>
            </form>
      ...
      {isScheduleModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between p-6 border-b border-[#e5e1d8] bg-[#FAF8F5]">
              <div>
                <p className="text-[10px] font-bold text-[#b5985b] uppercase tracking-widest mb-1">Weekly Calendar</p>
                <h3 className="font-serif text-xl font-bold text-[#033327]">Lab Schedule</h3>
              </div>
              <button 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#e5e1d8]">
                    <th className="py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Day</th>
                    <th className="py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Morning (08:00 - 12:00)</th>
                    <th className="py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Afternoon (13:00 - 17:00)</th>
                  </tr>
                </thead>
                <tbody className="text-sm">
                  <tr className="border-b border-[#e5e1d8]/50 hover:bg-[#FAF8F5] transition-colors">
                    ...
                    <td className="py-4 text-[#b5985b] font-medium">Open for Booking</td>
                  </tr>
                  <tr className="border-b border-[#e5e1d8]/50 hover:bg-[#FAF8F5] transition-colors">
                    <td className="py-4 font-bold text-[#033327]">Tuesday</td>
                    <td className="py-4 text-[#b5985b] font-medium">Open for Booking</td>
                    <td className="py-4 text-[#1a1a1a]">Grade 11 Programming</td>
                  </tr>
                  <tr className="border-b border-[#e5e1d8]/50 hover:bg-[#FAF8F5] transition-colors">
                    <td className="py-4 font-bold text-[#033327]">Wednesday</td>
                    <td className="py-4 text-[#1a1a1a]">Staff Training</td>
                    <td className="py-4 text-[#1a1a1a]">Database Maintenance</td>
                  </tr>
                  <tr className="border-b border-[#e5e1d8]/50 hover:bg-[#FAF8F5] transition-colors">
                  ...
                  <tr className="hover:bg-[#FAF8F5] transition-colors">
                    ...
                    <td className="py-4 text-gray-400">Closed for Maintenance</td>
                  </tr>
                </tbody>
              </table>
            </div>
