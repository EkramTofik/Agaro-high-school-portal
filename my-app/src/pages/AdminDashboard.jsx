...
/* ── Data ─────────────────────────────────────────────────── */
const quickActions = [
  { icon: <UserPlusIcon />, label: 'Manage Faculty', route: '/admin/faculty' },
  { icon: <MegaphoneIcon />, label: 'Post Announcement', route: '/admin/news' },
  { icon: <UploadIcon />, label: 'Manage Exam Results', route: '/admin/academic-records' },
  { icon: <UsersIcon />, label: 'Manage Alumni', route: '/admin/alumni' },
  { icon: <GridIcon />, label: 'Manage Teams', route: '/admin/teams' },
  { icon: <UsersIcon />, label: 'Manage Student Voice', route: '/admin/student-voices' },
  { icon: <ArchiveIcon />, label: 'Manage Honor Roll', route: '/admin/honor-roll' },
]
...
export default function AdminDashboard() {
  ...
  const [counts, setCounts] = useState({ faculty: 0, news: 0, records: 0, alumni: 0 })
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  useEffect(() => {
    Promise.all([api.get('/staff'), api.get('/news'), api.get('/academicRecord'), api.get('/alumni')])
      .then((responses) => {
        const count = (res) => res.data?.results ?? (Array.isArray(res.data?.data?.data) ? res.data.data.data.length : 0)
        setCounts({ faculty: count(responses[0]), news: count(responses[1]), records: count(responses[2]), alumni: count(responses[3]) })
      })
      .catch(() => setError('Could not load dashboard metrics.'))
      .finally(() => setLoading(false))
  }, [])
  const stats = [
    { label: 'Faculty Members', value: counts.faculty },
    { label: 'News Updates', value: counts.news },
    { label: 'Academic Records', value: counts.records },
    { label: 'Alumni Profiles', value: counts.alumni },
  ]
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#f4f1ec]">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-7 space-y-7">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {!loading && error && <p className="col-span-4 text-sm text-red-500">{error}</p>}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                <p className="text-sm text-gray-500 bg-white border border-[#e5e1d8] rounded-xl p-4">
                  No activity log is available from the backend yet.
                </p>
              </div>
            </div>
