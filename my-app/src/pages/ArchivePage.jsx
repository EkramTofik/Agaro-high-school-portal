...
export default function ArchivePage() {
  ...
  const totalPages = 1
  useEffect(() => {
    Promise.all([api.get('/news/published-news'), api.get('/academicRecord')]).then(([newsRes, recordsRes]) => {
      const news = newsRes.data?.data?.news || []
      setArticles(news.map((item) => ({ id: item._id, category: item.category || 'General', filter: item.category || 'General', date: item.publishedAt || item.createdAt, title: item.title, excerpt: item.summary || item.body, img: item.imageUrl })))
      const records = recordsRes.data?.data?.data || recordsRes.data?.data || []
      const years = {}; (Array.isArray(records) ? records : [records]).forEach((record) => { const year = (record.year || "CURRENT") + " RECORDS"; (years[year] ||= []).push(record.title || record.name || 'Academic Record') }); setArchiveYears(years)
    }).catch(() => { setArticles([]); setArchiveYears({}) }).finally(() => setLoading(false))
  }, [])

  const filters = useMemo(() => ['All Entries', ...new Set(articles.map((article) => article.filter))], [articles])
  const filtered =
